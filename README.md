...
### 🧰 공통 편집 기능
모든 모드에서 동일하게 제공되는 편집 도구입니다.

- **플로팅 팔레트**: 화면 어디서나 캐릭터·도형·경로를 꺼내 사용 (스크롤 불필요)
- **요소 추가 방식 2가지**: 무대로 끌어다 놓기(드래그) 또는 클릭 시 무대 중앙에 삽입
- **이동·크기·회전**: 드래그 이동, 모서리 손잡이로 크기, 위쪽 손잡이로 회전
- **중앙 안내선 + 자동 스냅**: 가로·세로 중앙선에 가까워지면 캔바처럼 가운데 정렬
- **실행취소/다시실행** (Ctrl+Z / Ctrl+Y), **방향키 이동**, **[ ] 회전**
- **우클릭 컨텍스트 메뉴** (복사·붙여넣기·삭제)
- **자동 저장** (localStorage) 및 헤더 저장 상태 표시
- **내보내기**: 전체 PDF · 항목별 이미지(JPG) · **공유 링크 복사** · JSON 저장/불러오기

### 🎵 안무 모드 (Choreography)
곡의 절 단위(#1, #2 …)로 무대 동선을 설계합니다.

- 블록마다 가사와 무대 그리드를 독립적으로 관리
- 추천 이동 패턴 8종 (직선·원형·대각선·삼각형·파형·S커브·8자·L자) — **도형 우선** 팔레트
- 커스텀 패턴 직접 그리기 (캔버스 드로잉, 모달)

### 🎬 장면 모드 (Scene)
연극·뮤지컬의 장면(scene)을 **장(章) 단위로 묶어** 기획합니다.

- **장 그룹**: 여러 장면을 1장·2장처럼 묶고, 제목을 바꾸거나 접고/펼치기
- 장면별 등장인물과 배치를 개별 관리
- 반응형 그리드(최대 3열)로 여러 장면을 한눈에 비교
- 30색 캐릭터 팔레트로 배우 구분

### 🔗 링크 공유
현재 동선 전체를 압축해 URL에 담아(별도 서버 없이) 복사합니다. 받은 사람이 링크를 열면 다른 기기에서도 동일한 동선을 그대로 볼 수 있습니다.
...
├── components/
│   ├── StageGrid.tsx         # 무대 그리드 (드롭·이동·크기·회전·스냅·안내선)
│   ├── DraggableElement.tsx  # 드래그/클릭 추가 가능 요소 (터치 지원)
│   ├── FloatingPalette.tsx   # 화면 하단 플로팅 요소 팔레트
